...
func assertBook(t *testing.T, book Book, expectedID int, expected NewBook) {
	...
	assert.Equal(t, expected.Description, book.Description)
	assert.Equal(t, expected.ISBN, book.ISBN)
	assert.Equal(t, expected.AuthorID, book.AuthorID)
	assert.Equal(t, expected.CategoryID, book.CategoryID)
	assert.Equal(t, expected.PublishedDate, book.PublishedDate)
}
